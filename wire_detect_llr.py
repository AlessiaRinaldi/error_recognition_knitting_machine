...
# PER COMPILARE: python wire_detect_llr.py --pos .\data\pos --neg .\data\neg --roi .\roi_mask.png --dir .\test --thr 0.0
...
def main():
    ...
    for p in sorted(paths):
        ...
        s_pos = abs(ncc(g, pp, mask=roi_rs)) # abs perchè sia +1 che -1 significativi
        ...
        cv2.putText(vis, f"filo={present}  LLR={llr:.3f} (pos={s_pos:.3f} neg={s_neg:.3f}) thr={args.thr}",
